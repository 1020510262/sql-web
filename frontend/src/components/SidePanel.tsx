...
export function SidePanel({ history, templates, onAddTemplate, onUseTemplate }: Props) {
  ...
  return (
    <section className="flex min-h-0 flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white/90 p-5 shadow-xl shadow-slate-200/70">
      ...
      <div className="mt-4 min-h-0 flex-1 overflow-y-auto pr-1">
        {tab === 'history' ? (
          ...
          <div className="space-y-3">
            ...
            {templates.map((item) => (
              <button key={item.id} onClick={() => onUseTemplate(item.sql_content)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left hover:border-slate-300">
                <div className="flex items-center justify-between gap-4">
